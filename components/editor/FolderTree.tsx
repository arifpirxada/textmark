...
const FolderTree: React.FC<PropType> = ({ folders, parentId = null, notes, toggleFolder }) => {
    ...
    return (
        <ul id={ `folder-${parentId}` } className={ `space-y-2 font-medium text-xs ${parentId ? "pl-2 hidden" : "pl-0"}` }>
            { children.map((folder) => (
                <li key={ folder.id }>
                    <FolderBtn folder={ folder } toggleFolder={ toggleFolder } />
                    {/* Recursively render children */ }
                    <FolderTree folders={ folders } parentId={ folder.id } notes={ notes } toggleFolder={ toggleFolder } />
                </li>
            )) }
